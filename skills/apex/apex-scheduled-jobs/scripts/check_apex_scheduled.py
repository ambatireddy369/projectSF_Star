...
Usage:
    python3 check_apex_scheduled.py [--manifest-dir path/to/metadata]
    python3 check_apex_scheduled.py --help
...
# Detects direct SOQL inside execute() — simplified heuristic: looks for
# [ SELECT in the same file alongside a Schedulable implements declaration.
# Full parse is not feasible in stdlib; this catches the most common inline case.
...
# Detects direct Http callout construction (HttpRequest, Http.send)
CALLOUT_RE = re.compile(
    r'\b(new\s+Http\b|new\s+HttpRequest\b|Http\(\)\.send\b)',
    re.IGNORECASE,
)

# Detects System.schedule() calls — useful for finding scheduling in unexpected places
SYSTEM_SCHEDULE_RE = re.compile(
    r'\bSystem\.schedule\s*\(',
    re.IGNORECASE,
)

# Detects DML keywords that could indicate inline DML in a Schedulable
...
def analyze_apex_file(path: Path) -> list[str]:
    ...
    # Only analyze files that implement Schedulable
    ...
    # 3. Warn if there is no dispatch call — possible inline logic
    ...
    if not has_dispatch and (has_inline_soql or has_inline_dml):
        issues.append(
            ...
            "Consider delegating heavy work to Batch Apex or a Queueable to "
            "avoid governor limit failures as data volume grows."
        )

    # 4. Warn on direct HTTP callouts
    if CALLOUT_RE.search(source):
        issues.append(
            f"{label}: Possible direct HTTP callout (HttpRequest / Http) detected "
            "in a Schedulable class. Callouts from Scheduled Apex throw a runtime "
            "CalloutException. Delegate callout work to a @future(callout=true) "
            "method or a Queueable implementing Database.AllowsCallouts."
        )

    # 5. Warn if System.schedule() is called inside what looks like execute()
    # Heuristic: both EXECUTE_METHOD_RE and SYSTEM_SCHEDULE_RE match the same file
...
def check_directory(manifest_dir: Path) -> list[str]:
    """Walk manifest_dir and analyze all .cls Apex files."""
    issues: list[str] = []
    ...
    if not apex_files:
        # Not necessarily an error — the checker may be run against a partial manifest
        return issues
    ...
    for apex_file in sorted(apex_files):
        try:
            ...
            continue
        if SCHEDULABLE_CLASS_RE.search(source):
            schedulable_count += 1
